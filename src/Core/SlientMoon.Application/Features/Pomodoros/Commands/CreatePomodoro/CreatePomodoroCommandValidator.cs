    ...
    public class CreatePomodoroCommandValidator : AbstractValidator<CreatePomodoroCommand>
    {
        public CreatePomodoroCommandValidator()
        {
            ...
            RuleFor(p => p.PomodoroTime)
                .NotEmpty().WithMessage("{PropertyName} is required.").WithErrorCode("PomodoroTime.NotEmpty")
                .NotEqual(0).WithMessage("{PropertyName} is not equal 0.").WithErrorCode("PomodoroTime.NotEqualZero")
                .NotNull().WithErrorCode("PomodoroTime.NotNull")
